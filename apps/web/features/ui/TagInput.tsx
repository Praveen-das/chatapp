        ...
        {canEdit &&
          (editTags ? (
            <div className="flex gap-2 items-center">
              ...
              {!tag ? (
                <div className="btn btn-xs btn-circle" onClick={() => toggleEditTags(false)}>
                  <Xmark />
                </div>
              ) : (
                <div className="btn btn-xs btn-circle" onClick={handleAddingTag}>
                  <Check />
                </div>
              )}
            </div>
          ) : !!tags?.length ? (
            <div className="btn btn-circle btn-xs" onClick={() => toggleEditTags((s) => !s)}>
              <Plus />
            </div>
          ) : (
            <div onClick={() => toggleEditTags((s) => !s)} className="flex items-center gap-2 cursor-pointer">
              <Plus />
              Add tags
            </div>
          ))}
