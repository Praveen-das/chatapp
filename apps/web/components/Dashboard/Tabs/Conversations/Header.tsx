...
function CreateGroupButton() {
  ...
  return <motion.div initial={false} layout>
    <button
      onClick={() => setDashboardTab("addMembersToGroup")}
    ...
    <>
      <Menu id="optionsMenu">
        {options.map(({ label, handler }) => (
          <Menu.Item key={label} onClick={handler}>{label}</Menu.Item>
        ))}
      </Menu>
      <span
        onClick={(e) =>
          setMenu({ reference: e.currentTarget, id: "optionsMenu", data: null })
        }
        tabIndex={0}
