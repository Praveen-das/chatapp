...
function Conversation({ conversation, isSelected }: IConve): React.JSX.Element {
  ...
  const { blockedByUsers } = useSocket()
  ...
  const member = conversation.members.find(member => member.id !== currentUser?.id)
  const user = users.find(connectedUser => connectedUser.id === member?.id)
