...
function GroupConversation({
  ...
  const from = conversation.members.find(m=>m.id === recentMessage?.from)?.username;
  const message = recentMessage
    ? from + ":" + recentMessage.message
      ...
      >
        ...
        <div className="min-w-0 w-full">
          <div className="flex gap-4 justify-between items-center">
            ...
            {recentMessage && (
              <label className="text-xs whitespace-nowrap" htmlFor="">
                {moment(new Date(recentMessage?.timestamp!)).format(
                  "LT"
                )}
              </label>
            )}
          ...
          <div className="flex justify-between items-center h-5 ">
            <h1 className="text-sm truncate w-2/3">{message}</h1>
            <div className="flex items-center duration-100">
              ...
              <span onClick={handleOpeningMenu} tabIndex={0} className="btn btn-circle btn-ghost w-6 h-6 min-h-6 -mr-1 duration-100 opacity-0 group-hover:opacity-100">
