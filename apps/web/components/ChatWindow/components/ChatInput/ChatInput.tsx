...
export function ChatInput() {
  ...
  const receiver = selectedConversation?.members.find(m => m.id !== user?.id!) || selectedUser?.id
  const isBlockedUser = blockedUsers.some(u => u.blockedId === receiver);

  return (
    <div className='flex flex-col w-full mx-auto bg-base-100 shadow-lg rounded-2xl'>
      {
        isBlockedUser ?
  ...
  return (
    <div className="flex justify-between items-center gap-4 w-full h-[60px] mx-auto p-4  ">
      ...
      <div className="flex items-center gap-4">
        <button className="btn btn-circle btn-sm btn-ghost" onClick={handleDeletingChat}>
        ...
        <button className="btn btn-circle btn-sm btn-ghost" onClick={handleForwadingChat}>
...
function Input(): React.ReactNode {
  ...
  const handleMessaging = () => {
    ...
    let conversation =
      ...
      conversations.find(s => s.members.find(m => m.id === selectedUser?.id!) && s.host === 'user') ||
    ...
    const payload = generateMessageTemplate(conversation, message);
  ...
  return (
    <>
      <Popover as="div" className="relative inline-block text-left ml-auto">
        ...
        <Popover.Panel ref={popoverRef} className="grid absolute bottom-10 -left-1/2 z-10 whitespace-nowrap rounded-md bg-base-100 shadow-lg ring-1 ring-black ring-opacity-5 overflow-hidden">
          {({ close }) => (
