      ...
      <div className={`flex-1 w-[calc((100vw-(1rem*2))/3)] h-full flex flex-col bg-gradient-to-t from-base-100 rounded-2xl overflow-hidden`}>
        ...
        <div className='flex gap-6 text-sm flex-col overflow-y-scroll py-6 no-scrollbar'>
          ...
          <div className='w-full flex flex-col gap-2 items-center '>
            <Avatar size='160px' onlineIndication={false} />
            <div className='w-full px-8'>
              <label className={`${editGroupName ? 'border-b-primary pl-0  pr-14' : 'border-b-transparent'} bg-transparent border-b-2 rounded-none textarea relative flex justify-center items-center w-full gap-2`}>
                {editGroupName ?
                  <>
                    ...
                    <div className="flex gap-1 items-center absolute right-0">
                      ...
                      <div onClick={handleEditGroupName} tabIndex={0} className="btn btn-circle btn-ghost btn-xs ">
                  ...
                  <div className='relative'>
                    <label className="mx-2" htmlFor="">{conversation?.displayName}</label>
                    <div onClick={handleEditGroupName} tabIndex={0} className="absolute btn btn-circle btn-ghost btn-xs ">
              ...
              {editDescription ?
                <>
                  ...
                  <div className="flex gap-1 items-center absolute right-0">
                    ...
                    <div onClick={handleEditGroupDescription} tabIndex={0} className="btn btn-circle btn-ghost btn-xs ">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5">
          ...
          <div className='w-full flex flex-col'>
            <div className='flex gap-4 px-8'>
              <label className='text-sm text-primary mb-2 ' htmlFor="">Group members</label>
              {conversation?.members.length}
            </div>
            <div className='flex gap-1 flex-col w-full '>
              <div tabIndex={0} onClick={() => toggleModal('addGroupMembersModal')} className='hover:bg-base-200 w-full flex items-center gap-4 px-8 py-3 cursor-pointer'>
              ...
              <div onClick={() => setProfileTab('inviteLink')} tabIndex={0} className='hover:bg-base-200 w-full flex items-center gap-4 px-8 py-3 cursor-pointer'>
                <div className='flex items-center justify-center w-[40px] h-[40px] bg-gray-500 rounded-full'>
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="size-5">
              ...
              {members.map((member, i) => (i < 5 && <Member key={member.id} member={member} />))}
              <div onClick={() => toggleModal('allMembers')} tabIndex={0} className='hover:bg-base-200 p-4 flex justify-center items-center cursor-pointer'>
                View all members
              </div>
            </div>
          </div>
          ...
          <div className='flex flex-col gap-2 px-8'>
            <div onClick={handleExitingGroup} tabIndex={0} className='btn btn-block text-white'>
              Exit Group
            </div>
          </div>
...
function Member({ member }: { member: IGroupMember }) {
  ...
  return (
    <div
      key={member.id}
      className='group hover:bg-base-200 w-full flex items-center gap-4 px-8 py-3 cursor-pointer'
