...
import { compressImage, generateConversation } from "../../../../helpers/helpers";
...
export function ChatInput() {
  ...
  const receiver = selectedConversation?.members.find(m => m !== user?.id!) || selectedUser?.userId
  const isBlockedUser = blockedUsers.some(u => u.blockedId === receiver);

  return (
    <div className='flex flex-col w-full mx-auto bg-zinc-700 rounded-2xl'>
      {
        isBlockedUser ?
          <BlockedUserNotification socketId={selectedConversation?.id!} /> :
...
function SelectedMessagesActions() {
  ...
  return (
    <div className="flex justify-between items-center gap-4 w-full h-[60px] mx-auto p-4 text-white ">
      ...
      <div className="flex items-center gap-4">
        <button className="btn btn-circle btn-sm btn-ghost" onClick={() => setModalState('deleteMessageModal')}>
        ...
        <button className="btn btn-circle btn-sm btn-ghost" onClick={() => setModalState('forwardMessageModal')}>
...
function Input(): React.ReactNode {
  ...
  const handleMessaging = () => {
    ...
    const payload = generateMessageTemplate(message);
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-1 w-full h-[60px] mx-auto p-4  text-white ">
        ...
        <AttachmentPicker />
        <input value={message} onChange={e => setMessage(e.target.value)} className="w-full h-full bg-transparent outline-none border-none ml-3" type="text" />
        <button
  ...
  return (
    <>
      <Popover as="div" className="relative inline-block text-left ml-auto">
        ...
        <Popover.Panel ref={popoverRef} className="grid absolute bottom-10 -left-1/2 z-10 whitespace-nowrap rounded-md bg-zinc-700 shadow-lg ring-1 ring-black ring-opacity-5 overflow-hidden">
          {({ close }) => (
            <InputButton className='btn btn-ghost' onInputChange={(file) => {
...
function BlockedUserNotification({ socketId }: { socketId: string }): React.ReactNode {
  return (
    <div className="flex justify-center items-center gap-4 w-full h-[60px] mx-auto p-4 text-white ">
      <label className="text-sm" htmlFor="">
        Can't send message to blocked user {socketId}
      </label>
    </div>
  );
}
